    ...
    <div class="row y-gap-10 items-center justify-between">
      <div class="col-auto">
        <div class="row x-gap-10 y-gap-5 items-center text-14 text-light-1">
          ...
          <div class="col-auto text-dark-1">Produk & Layanan</div>
        </div>
      </div>
    </div>
  </div>
...
<section class="layout-pt-md layout-pb-md">
  <div class="container">
    ...
    {{-- Kategori: Pendukung Tour --}}
    @if($pendukungCategories->count())
    <div class="sectionTitle -md mb-20">
      <h5 class="sectionTitle__text mt-5 sm:mt-0">Kategori Produk Pendukung Tour & Travel</>
    </div>

    <div class="category-grid">
      @foreach ($pendukungCategories as $category)
      <div class="category-card">
        <a href="{{ route('frontend.products.subcategories', $category->slug) }}">
          <img src="{{ asset($category->thumbnail) }}" alt="{{ $category->name }}">
          <div class="text-14 fw-600 text-dark-1 mt-10">{{ $category->name }}</div>
